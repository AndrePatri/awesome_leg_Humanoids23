...
\begin{figure}[h]
	...
	\caption{Data from multiple executions of the optimal take-off trajectory on the real robot. On top, an additional validation of the $i_q$ model (shown for the hip joint and a single jump). At the bottom, the measured currents (right) and torques (left) during  jumps, which are in line with the computed values shown in~Fig.~\ref{fig:takeoff_opt_data}.}
